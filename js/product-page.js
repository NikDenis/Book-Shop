import { reviewsProductPage } from "./data.js";
const reviewsProduct = JSON.parse(reviewsProductPage);

const reviewsList = document.querySelector(".reviews__list");

reviewsProduct.forEach(({ image, name, countBuy, discription }) => {
  const productEl = `
...
const recentlyViewed = JSON.parse(catalogBooks);

const recentlyViewedList = document.querySelector(".recently-viewed__list");

recentlyViewed.forEach(
  ({ reatingCount, image, name, author, discription, countPage, id }) => {
    if (id >= "005") {
      return;
    } else {
      const productEl = `
      <li class="card-book__item recently-viewed__item">
        <span class="book-item-reating-top">
          ...
          <span>${reatingCount}</span>
        </span>
        <a class="page-content-new-book-link card-book__link recently-viewed__link" href="">
          <img
        ...
        <div class="card-book__position-wrp recently-viewed__position-wrp">
          <div class="card-book__discription-wrp">
          <p class="page-content-new-book-author card-book__author recently-viewed__author">
          ${author}
          </p>
          <h4 class="page-content-new-book-name card-book__name recently-viewed__name">
            ${name}
          </h4>
          <p class="page-content-new-book-txt card-book__text recently-viewed__txt">
            ${discription}
          </p>
          </div>
          <div class="card-book__btn-wrp">
            <button class="page-content-book-btn card-book__btn ">
            Подробнее
            </button>
      ...
      recentlyViewedList.insertAdjacentHTML("beforeend", productEl);
    }
  }
);
const imageBook = document.querySelector(".book__img");
const nameBook = document.querySelector(".book__name");
const linkNameBook = document.querySelector(".link-name-book");
const reatingBook = document.querySelector(".book__reating-txt");
const authorBook = document.querySelector(".book__author");
...
authorBook.innerText = localStorage.getItem("author");
countPageBook.innerText = parseInt(
  localStorage.getItem("countPage").replace(/\D+/g, "")
);
